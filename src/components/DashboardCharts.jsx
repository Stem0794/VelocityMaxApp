...
export default function DashboardCharts({ metrics, issues, selectedStatuses, setSelectedStatuses, loadingHistory, historyProgress }) {
  ...
  const renderChart = id => {
    switch (id) {
      case 'velocity':
        return <ChartCard key={id} {...cardProps(id, 'Weekly velocity')} summary="Points and ticket throughput by ISO week." details="Bars show delivered story points; the ticket line shows completed issue count. The dashed line is a four-week rolling average of tickets."><VelocityChart data={metrics.velocityData} /></ChartCard>;
      case 'cycle-compare':
        return metrics.cycleComparison.length >= 2 ? <ChartCard key={id} {...cardProps(id, 'Cycle comparison')} summary="Delivery and completion across recent cycles." details="Compare points, ticket count and completion percentage for up to eight recent cycles. The dot marks the current cycle."><CycleComparisonChart data={metrics.cycleComparison} /></ChartCard> : null;
      case 'cycle-times':
        return <ChartCard key={id} {...cardProps(id, 'Issue cycle times')} summary="Completed issues plotted by delivery date and time in progress." details="Each point is a completed issue. Red points exceeded 14 days in progress and are useful candidates for retro review."><CycleTimesChart data={metrics.cycleTimeData} /></ChartCard>;
      case 'burnup':
        return <ChartCard key={id} {...cardProps(id, 'Burn-up')} summary="Cumulative scope versus completed story points." details="Scope rises when estimated issues are created. Completed work rises when those issues finish. A widening gap signals growing remaining scope."><BurnupChart data={metrics.burnupData} /></ChartCard>;
      case 'burndown':
        return <ChartCard key={id} {...cardProps(id, 'Sprint burndown', { actions: metrics.uniqueCycles.length ? <select className="chart-inline-select" value={selectedCycle} onChange={event => setSelectedCycle(event.target.value)} aria-label="Sprint cycle">{metrics.uniqueCycles.map(cycle => <option key={cycle.number} value={cycle.number}>Cycle {cycle.number}{cycle.number === metrics.currentCycleNumber ? ' · current' : ''}</option>)}</select> : null })} summary="Remaining points versus ideal sprint pace." details="A flat remaining line means no point-bearing work closed during that period. Falling faster than ideal means the sprint is ahead of a linear pace.">{metrics.uniqueCycles.length ? <BurndownChart data={burndownData} /> : <div className="chart-empty">No cycle data in the current scope.</div>}</ChartCard>;
      case 'lead-time':
        return <ChartCard key={id} {...cardProps(id, 'Lead-time distribution')} summary="Time from issue creation to completion." details="Buckets are inclusive at their displayed upper bounds: ≤3d, >3–7d, >7–14d, >14–30d and >30d."><LeadTimeChart data={metrics.leadTimeHistogram} /></ChartCard>;
      ...
      case 'cfd':
        return <ChartCard key={id} {...cardProps(id, 'Cumulative flow')} summary="Weekly inventory across backlog, active work and terminal states." details="A widening In Progress band can indicate WIP accumulation. Done grows cumulatively as issues complete."><CumulativeFlowChart data={metrics.cumulativeFlowData} /></ChartCard>;
      case 'prediction':
        return metrics.predictionResult ? <ChartCard key={id} {...cardProps(id, 'Scope forecast')} summary={`${metrics.predictionResult.remaining} points remain in the current scope.`} details="Forecasts use the most recent four weeks of point velocity. Optimistic uses the best recent week, average uses the mean and pessimistic uses the slowest non-zero week."><PredictionChart data={metrics.predictionResult} /></ChartCard> : null;
      case 'issues':
        return <ChartCard key={id} {...cardProps(id, 'Issues', { fullWidth: true })} summary="Search, sort and export the issues in the current scope." details="CSV export neutralizes spreadsheet-formula prefixes in user-controlled text fields."><IssuesTable issues={issues} /></ChartCard>;
      default:
        return null;
    }
  };
